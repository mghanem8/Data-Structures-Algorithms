...
public class ExternalChainingHashMap<K, V> {
    ...
    public V put(K key, V value) {
        if (key == null || value == null) {
            throw new IllegalArgumentException("Key value pair cannot contain null value.");
        }
    ...
    public boolean containsKey(K key) {
        if (key == null) {
            throw new IllegalArgumentException("Cannot check contains a null key.");
        }
        int index = Math.abs(key.hashCode() % table.length);
        ExternalChainingMapEntry<K, V> temp = table[index];
        while (temp != null) {
     ...
     */
    public Set<K> keySet() {
        Set<K> set = new HashSet<K>(table.length);
        for (ExternalChainingMapEntry<K, V> tempEntry: table) {
            while (tempEntry != null) {
                set.add(tempEntry.getKey());
    ...
    public List<V> values() {
        List<V> arrayList = new ArrayList<V>(table.length);
        for (ExternalChainingMapEntry<K, V> tempEntry: table) {
            while (tempEntry != null) {
                arrayList.add(tempEntry.getValue());
                tempEntry = tempEntry.getNext();
            }
        }
        return arrayList;
    ...
    public void resizeBackingTable(int length) {
        if (length < size) {
            throw new IllegalArgumentException("Cannot resize, new length is too small.");
        }
        ExternalChainingMapEntry<K, V>[] newTable = new ExternalChainingMapEntry[length];
        for (ExternalChainingMapEntry tempEntry : table) {
            while (tempEntry != null) {
                int index = Math.abs(tempEntry.getKey().hashCode() % newTable.length);
                newTable[index] = new ExternalChainingMapEntry<K, V>((K) tempEntry.getKey(),
                        (V) tempEntry.getValue(), newTable[index]);
                tempEntry = tempEntry.getNext();
            }
        }
        table = newTable;
